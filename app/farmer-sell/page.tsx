...
export default function SellingPage() {
  ...
  const updateQty = (id: number, qty: number) => {
    setSelected((prev) => ({ ...prev, [id]: Math.max(5, qty) })); // min 5 kg
  };
  ...
  const whatsappMessage = () => {
    ...
    selectedList.forEach((v) => {
      ...
      const price = (v.price * qty).toFixed(2);
      msg += `- ${v.name}: ${qty} kg – ₹${price}\n`;
    });
    const total = selectedList.reduce((sum, v) => sum + v.price * selected[v.id], 0).toFixed(2);
    ...
    <main className="bg-[#f6faf7] min-h-screen text-gray-800">
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="flex justify-start mb-8">
          <a href="/" className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold transition">
            Back to the Home page!
          </a>
        </div>

        <h1 className="text-4xl font-bold text-green-700 mb-6 text-center">Sell your vegetables!</h1>
                ...
                <div className="flex-1">
                  ...
                  {selected[veg.id] && (
                    <div className="mt-4">
                      ...
                      <input type="number" min={5} step={1} value={qty} onChange={(e) => updateQty(veg.id, Number(e.target.value))} className="block mt-1 w-28 border border-green-200 rounded-lg px-3 py-2 text-gray-800" />
                      <p className="mt-1 text-gray-700 font-semibold">Expected Payment: ₹{vegPrice}</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-green-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-lg rounded-2xl p-6 shadow-xl">
            ...
            <div className="mt-4 font-bold text-lg">
              Total Expected Payment: ₹{selectedList.reduce((sum, v) => sum + v.price * selected[v.id], 0).toFixed(2)}
            </div>

            {/* Farmer Form */}
            <div className="space-y-3 mt-4">
              <input type="text" placeholder="Farmer Name" className="w-full border border-green-200 px-3 py-2 rounded-lg text-gray-800" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              ...
              <textarea placeholder="Pickup Address" className="w-full border border-green-200 px-3 py-2 rounded-lg text-gray-800" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
              <input type="text" placeholder="Pincode" className="w-full border border-green-200 px-3 py-2 rounded-lg text-gray-800" value={form.pincode} onChange={(e) => setForm({ ...form, pincode: e.target.value })} />
            ...
            <div className="flex justify-between mt-6">
              ...
              <button onClick={handleProceed} className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg">Proceed To Sell</button>
            </div>
          </div>
        </div>
      )}
